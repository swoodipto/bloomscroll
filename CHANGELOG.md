...
- **TikTok/Reels-style feed mode** (all five phases of `FEED_UX_PLAN.md`).
  ...
  **Refresh** button that loads the next batch and returns to the top.
  ...
  - *Tap vs swipe* — a card click only opens the note if the pointer moved less
    than 10px between `pointerdown` and `pointerup`, so a swipe that starts on
    the card does not open it. An explicit "Open note" button shipped alongside
    this at first and was later removed: the drift guard turned out to be enough
    on its own, and the button crowded the card.
...
- **Visual design pass.** The feed moved from plain bordered cards to a
  ...
  - *Paper card* — a sheet that fills its card up to 608x900, with a fibrous
    texture generated by an inline SVG `feTurbulence` filter rather than an
    image asset, layered shadows, embossed text-shadow on descendants, and
    `--radius-m` corners. It began as a fixed 400x500 panel; the caps-plus-100%
    sizing lets it adapt to the window and to phone screens, where `.is-mobile`
    padding keys off Obsidian's own navbar and keyboard variables.
